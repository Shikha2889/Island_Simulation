...
def start_simulation(rh_population, s_population, p_population):
    red_hawks_counter = 0
    snake_counter = 0
    puma_counter = 0
    ...
    for _ in range(EXPERIMENT_CNT):
        ...
        animals_in_island = populate_island(rh_population, s_population, p_population,
    ...
    print(
        f"""For the given population of {rh_population},{s_population},{p_population} only {animal_counter} survived""")
...
def main():
    ...
    with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
        pool.starmap(start_simulation, animal_populations)
